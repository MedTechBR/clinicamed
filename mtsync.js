...
const NUCLEO = (() => {
  ...
  function monta(spec, mapa, ant) {
    ...
    if (spec.tipo === "lista") {
      ...
      Object.keys(mapa).filter(id => !vistos.has(id)).sort().forEach(id => out.push(mapa[id]));
...
const MTS = {
  ...
  async iniciar(cfg) {
    CFG = cfg; injetaCSS();
    if (cfg.cores) Object.keys(cfg.cores).forEach(k => H.style.setProperty("--mts-" + k, cfg.cores[k]));
    telaEspera();
